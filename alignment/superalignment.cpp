...
void SuperAlignment::splitPartitions(Params &params) {
    ...
    auto calcRate = [&](Alignment* aln, vector<bool>& isConst) {
        vector<double> rates;
        vector<vector<vector<int>>> sequences(aln->getNSite());
        vector<bool> isConstPattern(aln->size(), false);
        for (int i = 0; i < aln->size(); ++i) {
            Pattern p = aln->at(i);
            if (p.isConst()) {
                isConstPattern[i] = true;
                continue;
            }
        ...
        for (int i = 0; i < aln->getNSite(); ++i) {
            ...
            isConst.push_back(isConstPattern[aln->getPatternID(i)]);
        }
        return rates;
    };
    ...
    while (!q.empty()) {
        ...
        cout << "Splitting " << aln->name << " with " << aln->getNPattern() << " patterns and " << aln->getNSeq() << " sequences" << endl;
        // calculate rates by TIGER
        std::vector<bool> isConst;
        std::vector<double> rates = calcRate(aln, isConst);
        ...
        cout << "Split " << aln->name << " into " << subAlns.size() << " parts" << endl;
    }
    system(("rm -rf " + prefixPath).c_str());
